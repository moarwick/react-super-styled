...
var getCss = function getCss(props) {
  ...
  var xsShow = !hide && (show || smHide || mdHide || lgHide);
  var xsHide = hide || !xsShow && (smShow || mdShow || lgShow);
  var isSmRules = smHide || smShow;
  var isMdRules = mdHide || mdShow;
  var isLgRules = lgHide || lgShow;

  // prettier-ignore
  return css(['', ' ', ' ', ' ', ' ', ''], xsHide && 'display: none;', xsShow && toDisplayCss(false, show), isSmRules && theme.MEDIA_XS_MIN + ' { ' + toDisplayCss(smHide, smShow) + ' }', isMdRules && theme.MEDIA_SM_MIN + ' { ' + toDisplayCss(mdHide, mdShow) + ' }', isLgRules && theme.MEDIA_MD_MIN + ' { ' + toDisplayCss(lgHide, lgShow) + ' }');
};
